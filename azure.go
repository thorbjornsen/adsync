...
type AzureGroupServicePrincipals struct {
	OdataContext string `json:"@odata.context"`
	Value        []struct {
		Id                                 string        `json:"id"`
		DeletedDateTime                    interface{}   `json:"deletedDateTime"`
		...
		LoginUrl                           interface{}   `json:"loginUrl"`
...
type Group struct {
	...
	AzServicePrincipals []AzureGroupServicePrincipals
}
...
func (a *Azure) getGroupServicePrincipals(id string) (AzureGroupServicePrincipals, AzureError) {
	...
	//if len(next) == 0 {
	rurl = "https://graph.microsoft.com/v1.0/groups/" + id + "/members/microsoft.graph.servicePrincipal"
	//} else {
	//	rurl = next
	//}
	...
	if err != nil {
		logger.Debug("Problem creating the request: ", err)
		return AzureGroupServicePrincipals{}, AzureError{Err: err}
	}
	...
	if resp.StatusCode != http.StatusOK {
		logger.Debug("Unexpected status code: ", resp.Status)
		return AzureGroupServicePrincipals{}, AzureError{resp.Status, resp.StatusCode, errors.New("Expected " + strconv.Itoa(http.StatusOK))}
	}

	//
	// Read/verify the returned body
	//
	servicePrincipals := AzureGroupServicePrincipals{}

	if body, err := ioutil.ReadAll(resp.Body); err != nil {
		logger.Warn("Problem reading the result: ", err)
	} else if err = json.Unmarshal(body, &servicePrincipals); err != nil {
		logger.Warn("Problem unmarshaling the result: ", err)
		logger.Debug("Result: ", string(body))
	} else {
		logger.Debug("Result: ", string(body))
	...
	//for _, servicePrincipal := range servicePrincipals.Value {
	//	logger.Info("Service principal: ", servicePrincipal.DisplayName)
	//}

	//
	// Check for nested group(s) hiding among the members
	//
	// TODO: Check SP validation based on returned data
	// Make a copy of the member array
	//check := servicePrincipals.Value

	// Clear the member array
	//servicePrincipals.Value = servicePrincipals.Value[:0]
	...
	return servicePrincipals, AzureError{}
}
...
func (a *Azure) GetGroupMembers(id string) AzureError {
	...
	if config.Azure.SyncServicePrincipals {
		if servicePrincipals, err := a.getGroupServicePrincipals(id); !err.Ok() {
			return err
		} else {
			if len(servicePrincipals.Value) > 0 {
				logger.Info("Fetched ", len(servicePrincipals.Value), " Azure Service Principals for: ", group.AzGroup.DisplayName)
			}
		}
	}
...
func (a *Azure) GetAllGroupMembers(id string) AzureError {
	...
	if config.Azure.SyncServicePrincipals {
		if servicePrincipals, err := a.getGroupServicePrincipals(id); !err.Ok() {
			...
			group.AzServicePrincipals = append(group.AzServicePrincipals, servicePrincipals)
			a.Groups[id] = group
